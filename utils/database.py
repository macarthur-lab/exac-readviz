...
_readviz_db = playhouse.pool.PooledMySQLDatabase(
    'exac_readviz',
    user='root', host='dmz-exac-dev.broadinstitute.org', port=3307)
...
class _SharedVariantFields(_SharedMeta):
    ...
    minrep_pos = peewee.IntegerField()
    minrep_ref= peewee.CharField(max_length=MAX_ALLELE_SIZE)
    minrep_alt = peewee.CharField(max_length=MAX_ALLELE_SIZE)
...
class Variant(_SharedVariantFields):
    ...
    class Meta:
        indexes = (
            (('chrom', 'minrep_pos', 'minrep_ref', 'minrep_alt', 'het_or_hom'), True), # True means unique index
        )
...
class Sample(_SharedVariantFields):
    ...
    hc_started = peewee.BooleanField(default=0, index=True)
    ...
    class Meta:
        indexes = (
            (('chrom', 'minrep_pos', 'minrep_ref', 'minrep_alt', 'het_or_hom', 'sample_id'), True), # True means unique index
        )


def _create_table(db, model, indexes=tuple(), safe=True):
    ...
    # this is commented out to avoid
...
def init_db():
    ExacCallingInterval.create_table(fail_silently=True)
