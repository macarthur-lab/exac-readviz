...
run_query(("update sample as s join variant as v on "
           "v.chrom=s.chrom and v.pos=s.pos and v.ref=s.ref and v.alt=s.alt and v.het_or_hom=s.het_or_hom "
           "set s.finished=0, s.hc_error_code=s.hc_error_code+10 "
           "where s.hc_error_code IN (2001, 2009, 3001, 4000) "
...
sql_is_overlapping = []
sql_is_not_overlapping = []
with open("data/top_intervals.txt") as f:
    for line in f:
        ...
        sql_is_not_overlapping.append( "(chrom <> '%(i_chrom)s' or start_pos > %(i_end)s or end_pos < %(i_start)s )" % locals() )
...
#run_query(("select * from parallelize.python3_4_generate_HC_bams_py_i200 "
#           "where finished=0 and job_id is NULL and ( %s )") % " and ".join(sql_is_not_overlapping))


# disable intervals that are not overlapping the intervals of interest
#run_query(("update parallelize.python3_4_generate_HC_bams_py_i200 set job_id=-1 "
#           "where finished=0 and job_id is NULL and ( %s )") % " and ".join(sql_is_not_overlapping))
...
# update parallelize.python3_4_generate_HC_bams_py_i200 set job_id=-1
#  where finished=0 & job_id is NULL and ( (chrom='1' and start_pos<end and end_pos>start) or )"
